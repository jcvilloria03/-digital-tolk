...
class BookingRepository extends BaseRepository
{
    ...
    public function store($user, $data)
    {
        ...
        $customerRoleId = env('CUSTOMER_ROLE_ID');

        if ($user->user_type != $customerRoleId) {
        ...
        if ($user->user_type == $customerRoleId) {
            ...
            if ($job->certified != null) {
                $data['job_for'][] = ($job->certified == 'both') ? ['normal', 'certified'] : (($job->certified == 'yes') ? 'certified' : $job->certified);
            }
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function jobToData($job)
    {
        ...
        if ($job->certified != null) {
            ...
            $data['job_for'] = array_merge($data['job_for'], $certifiedMapping[$job->certified] ?? [$job->certified]);
        }
    ...
    public function jobEnd($post_data = array())
    {
        ...
        $user = $job->user()->get()->first();
        $this->sendEmailWithSession($job, $user, 'faktura');
        ...
        $user = $tr->user()->first();
        $this->sendEmailWithSession($job, $user, 'lön');
    ...
    public function sendEmailWithSession($job, $user, $forText){
        $name = $user->name;
        $email = $user->email;
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        $translatorType = ($jobType == 'paid') ? 'professional' :
                        ($jobType == 'rws') ? 'rwstranslator' :
                        ($jobType == 'unpaid') ? 'volunteer' : '';
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function sendChangedDateNotification($job, $old_time)
    {
        ...
        $this->sendEmail($email, $name, $subject, $job, $old_time, $user);

        $translator = Job::getJobsAssignedTranslatorDetail($job);

        $this->sendEmail($translator->email, $translator->name, $subject, $job, $old_time, $translator);

    }

    public function sendEmail($email, $name, $subject, $job, $old_time, $user)
    {
        $data = [
            'user'     => $user,
            'job'      => $job,
